...
import { View, Text, ScrollView, TouchableOpacity, TextInput, Alert, Modal, ActivityIndicator, Platform, Image } from 'react-native';
...
const ZoomableImage = ({ uri }: { uri: string }) => {
  ...
  const handleDoubleTap = () => {
    ...
    if (now - lastTap.current < 300) {
      // Double tap — close not available in this version, just reset
    }
    lastTap.current = now;
  };
...
export default function TuitionPostProgressScreen({ navigation }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload: any = {
        ...
        type: mode,
        comments,
        entries: classSubjects.map((s, idx: number) => {
          const entry: any = {
            subject_id: s.id,
            subject: s.name,
  ...
  const sectionCard = (children: React.ReactNode, icon: string, title: string, subtitle: string, color?: string) => {
    ...
    const soft = (c + '22');
    return (
    <View style={{ backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#F1F5F9', borderRadius: 24, padding: 20, marginBottom: 16 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
        ...
        <View>
          <Text style={{ fontSize: 14, fontWeight: '900', color: '#111827' }}>{title}</Text>
          <Text style={{ fontSize: 11, fontWeight: '600', color: '#9CA3AF' }}>{subtitle}</Text>
        </View>
      </View>
      {children}
    </View>
        ...
        <View style={{ paddingHorizontal: 20, paddingBottom: 40 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', paddingTop: 8, paddingBottom: 8 }}>
            <TouchableOpacity onPress={() => navigation.goBack()}
              style={{ backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#F1F5F9', width: 40, height: 40, borderRadius: 14, alignItems: 'center', justifyContent: 'center', marginRight: 14 }} activeOpacity={0.7}>
              <MaterialCommunityIcons name="arrow-left" size={22} color="#374151" />
            </TouchableOpacity>
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 12, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 2, color: '#9CA3AF' }}>Tuition</Text>
              <Text style={{ fontSize: 28, fontWeight: '900', letterSpacing: -0.5, marginTop: 2, color: '#111827' }}>Post Marks</Text>
              <Text style={{ fontSize: 12, fontWeight: '600', color: '#9CA3AF', marginTop: 2 }}>Record progress & test results</Text>
            </View>
            <View style={{ backgroundColor: themeColor, width: 60, height: 60, borderRadius: 16, alignItems: 'center', justifyContent: 'center', elevation: 4 }}>
              <MaterialCommunityIcons name={mode === 'progress' ? 'chart-line' : 'clipboard-check-outline'} size={28} color="white" />
            </View>
          </View>

          <View style={{ flexDirection: 'row', backgroundColor: '#F1F5F9', borderRadius: 18, padding: 5, marginBottom: 16 }}>
          ...
          {sectionCard(
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 10 }}>
              <TouchableOpacity activeOpacity={0.8} onPress={() => { setSelectedBatchId(null); setSelectedStudent(null); }}
                ...
                <Text style={{ fontWeight: '800', fontSize: 13, color: selectedBatchId === null ? 'white' : '#374151' }}>All Classes</Text>
              </TouchableOpacity>
              {batches.map(b => (
                <TouchableOpacity key={b.id} activeOpacity={0.8} onPress={() => { setSelectedBatchId(b.id); setSelectedStudent(null); }}
                  ...
                  <Text style={{ fontWeight: '800', fontSize: 13, color: selectedBatchId === b.id ? 'white' : '#374151' }}>{b.name}</Text>
                </TouchableOpacity>
              ))}
              {batches.length === 0 && !loadingMeta && (
                <Text style={{ fontWeight: '600', fontSize: 13, color: '#9CA3AF', paddingVertical: 12 }}>No classes available</Text>
              )}
            </ScrollView>,
          ...
          {sectionCard(
            <TouchableOpacity activeOpacity={0.7} onPress={() => setShowStudentPicker(true)}
              ...
              <Text style={{ flex: 1, fontWeight: '700', fontSize: 14, color: selectedStudent ? 'white' : '#374151' }}>
          ...
          {sectionCard(
            <TouchableOpacity activeOpacity={0.8} onPress={() => setShowDatePicker(true)}
              ...
              <Text style={{ flex: 1, fontWeight: '800', fontSize: 15, color: '#111827', marginLeft: 12 }}>{formatDisplay(date)}</Text>
            ...
            <View style={{ backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#F1F5F9', borderRadius: 24, padding: 20, marginBottom: 16 }}>
              ...
              {classSubjects.length === 0 ? (
                <View style={{ paddingVertical: 24, alignItems: 'center' }}>
                  ...
                  <Text style={{ color: '#9CA3AF', fontWeight: '700', fontSize: 13, marginTop: 8 }}>No subjects mapped to this class yet</Text>
                </View>
              ) : (
                classSubjects.map(s => (
                  <View key={s.id} style={{ marginTop: 16, padding: 16, borderRadius: 16, backgroundColor: themeSoft + '0.05)', borderWidth: 2, borderColor: themeSoft + '0.15)' }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
                      ...
                      <Text style={{ fontWeight: '900', fontSize: 14, color: '#111827', flex: 1 }}>{s.name}</Text>
                    ...
                    {mode === 'progress' ? (
                      <View>
                        <Text style={{ fontSize: 10, fontWeight: '700', color: '#6B7280', marginBottom: 6, marginLeft: 2 }}>ON-TRACK STATUS</Text>
                      ...
                      <View style={{ flexDirection: 'row', gap: 12 }}>
                        <View style={{ flex: 1 }}>
                          <Text style={{ fontSize: 10, fontWeight: '700', color: '#6B7280', marginBottom: 4, marginLeft: 4 }}>MARKS</Text>
                          <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFFFFF', borderWidth: 2, borderColor: themeSoft + '0.2)', borderRadius: 16, paddingHorizontal: 14, height: 48 }}>
                            ...
                            <TextInput style={{ flex: 1, fontWeight: '700', fontSize: 15, marginLeft: 8, color: '#111827' }}
                        ...
                        <View style={{ flex: 1 }}>
                          <Text style={{ fontSize: 10, fontWeight: '700', color: '#6B7280', marginBottom: 4, marginLeft: 4 }}>OUT OF</Text>
                          <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFFFFF', borderWidth: 2, borderColor: themeSoft + '0.2)', borderRadius: 16, paddingHorizontal: 14, height: 48 }}>
                      ...
                      <View style={{ marginTop: 12 }}>
                        <Text style={{ fontSize: 10, fontWeight: '700', color: '#6B7280', marginBottom: 4, marginLeft: 4 }}>GRADE</Text>
                        <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFFFFF', borderWidth: 2, borderColor: themeSoft + '0.2)', borderRadius: 16, paddingHorizontal: 14, height: 48 }}>
                      ...
                      <View style={{ marginTop: 12 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                          <Text style={{ fontSize: 10, fontWeight: '700', color: '#6B7280', marginLeft: 2 }}>TEST PAPER</Text>
                        ...
                        {(subjectFiles[String(s.id)] || []).length === 0 ? (
                          <Text style={{ fontSize: 11, fontWeight: '600', color: '#9CA3AF', marginLeft: 2 }}>
          ...
          {selectedStudent && (
            <View style={{ backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#F1F5F9', borderRadius: 24, padding: 20, marginBottom: 16 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
                ...
                <View>
                  <Text style={{ fontSize: 14, fontWeight: '900', color: '#111827' }}>Comments</Text>
                  <Text style={{ fontSize: 11, fontWeight: '600', color: '#9CA3AF' }}>Overall remarks (optional)</Text>
                </View>
              </View>
              <View style={{ backgroundColor: themeSoft + '0.05)', borderWidth: 2, borderColor: themeSoft + '0.15)', borderRadius: 16, paddingHorizontal: 16, paddingTop: 14 }}>
                <TextInput style={{ fontWeight: '600', fontSize: 14, lineHeight: 20, paddingBottom: 12, color: '#374151' }}
          ...
          {selectedStudent && (
            <View style={{ backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#F1F5F9', borderRadius: 24, padding: 20, marginBottom: 16 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
                ...
                {loadingHistory && <ActivityIndicator size="small" color={themeColor} style={{ marginLeft: 'auto' }} />}
              </View>

              {history.length === 0 ? (
                <View style={{ paddingVertical: 24, alignItems: 'center' }}>
                  <MaterialCommunityIcons name="file-chart-outline" size={40} color="#CBD5E1" />
                  <Text style={{ color: '#9CA3AF', fontWeight: '700', fontSize: 13, marginTop: 8 }}>
                ...
                history.map((h: any) => {
                  ...
                  <View key={h.id}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: atts.length > 0 ? 0 : 1, borderBottomColor: '#F1F5F9' }}>
                      ...
                      <Text style={{ flex: 1, fontWeight: '800', fontSize: 14, color: '#111827' }}>{h.subject?.name || h.subject || 'Subject'}</Text>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              {classStudents.length === 0 ? (
                <View style={{ paddingVertical: 40, alignItems: 'center' }}>
                  ...
                  <Text style={{ color: '#9CA3AF', fontWeight: '700', fontSize: 14, marginTop: 12 }}>
                ...
                <TouchableOpacity key={st.id} activeOpacity={0.7} onPress={() => { setSelectedStudent(st); setShowStudentPicker(false); }}
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontWeight: '900', fontSize: 14, color: '#111827' }}>{st.name}</Text>
                    <Text style={{ fontWeight: '700', fontSize: 12, color: '#9CA3AF' }}>
