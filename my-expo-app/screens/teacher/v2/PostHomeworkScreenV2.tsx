...
import { View, Text, ScrollView, TouchableOpacity, TextInput, Image, ActivityIndicator, Alert, Modal, Platform, StyleSheet, PanResponder, Dimensions, RefreshControl } from 'react-native';
...
// ── Reusable styled primitives (V2 glassmorphic) ──
const PickerHeader = ({ title, onClose, right, accent }: {
  title: string; onClose: () => void; right?: React.ReactNode; accent?: string;
}) => (
  <View style={{ paddingHorizontal: 20, paddingVertical: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.6)' }}>
    <TouchableOpacity onPress={onClose} style={{ width: 42, height: 42, borderRadius: 14, backgroundColor: 'rgba(255,255,255,0.92)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', alignItems: 'center', justifyContent: 'center' }}>
      <MaterialCommunityIcons name="close" size={22} color={TEXT_PRIMARY} />
    </TouchableOpacity>
    <Text style={{ fontSize: 17, fontWeight: '700', color: TEXT_PRIMARY }}>{title}</Text>
    {right ? right : <View style={{ width: 42 }} />}
  </View>
);

const Survey = React.memo(({ label, value, onChange, placeholder, multiline, icon }: {
  ...
  <View style={{ marginBottom: 18 }}>
    <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
      ...
      <Text style={{ fontSize: 12, fontWeight: '800', marginLeft: 8, textTransform: 'uppercase', letterSpacing: 1, color: '#B45309' }}>{label}</Text>
    </View>
    <View style={{ flexDirection: 'row', alignItems: 'flex-start', backgroundColor: 'rgba(255,255,255,0.92)', borderRadius: 16, borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', overflow: 'hidden' }}>
      <View style={{ backgroundColor: brand, width: 6, minHeight: 52 }} />
      <TextInput
        style={{ flex: 1, paddingHorizontal: 14, paddingVertical: multiline ? 14 : 15, fontSize: 14, fontWeight: '700', minHeight: multiline ? 140 : 52, color: TEXT_PRIMARY, textAlignVertical: multiline ? 'top' as const : 'center' }}
        placeholder={placeholder} placeholderTextColor={TEXT_MUTED}
...
const SurveyTag = React.memo(({ label, value, onChange, placeholder, multiline, icon }: {
  label: string; value: string; onChange: (t: string) => void; placeholder: string; multiline?: boolean; icon: string;
}) => (
  <View style={{ marginBottom: 18 }}>
    <Text style={{ fontSize: 11, fontWeight: '800', marginBottom: 6, marginLeft: 2, color: TEXT_MUTED }}>{label}</Text>
    <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.92)', borderRadius: 16, borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', overflow: 'hidden' }}>
      ...
      <TextInput
        style={{ flex: 1, paddingHorizontal: 14, paddingVertical: multiline ? 13 : 14, fontSize: 14, fontWeight: '700', minHeight: multiline ? 140 : 46, color: TEXT_PRIMARY, textAlignVertical: multiline ? 'top' as const : 'center' }}
        placeholder={placeholder} placeholderTextColor={TEXT_MUTED}
        value={value} onChangeText={onChange} multiline={multiline}
...
const Chip = React.memo(({ label, active, onPress }: { label: string; active: boolean; onPress: () => void }) => (
  <TouchableOpacity onPress={onPress} activeOpacity={0.7}
    style={{ paddingHorizontal: 14, paddingVertical: 9, borderRadius: 12, marginRight: 8, flexDirection: 'row', alignItems: 'center', backgroundColor: active ? brand : '#F3F4F6' }}>
    {active && <MaterialCommunityIcons name="check" size={14} color="#7c2d12" />}
    <Text style={{ fontWeight: '800', fontSize: 12, marginLeft: active ? 4 : 0, color: active ? '#7c2d12' : TEXT_MUTED }}>{label}</Text>
  </TouchableOpacity>
));

const Card = React.memo(({ children }: { children: React.ReactNode }) => (
  <View style={{ backgroundColor: 'rgba(255,255,255,0.92)', borderRadius: BORDER_RADIUS, padding: 18, borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.06, shadowRadius: 12 }}>
    {children}
  </View>
));
...
const PickerModal = React.memo(({ visible, onClose, title, children }: {
  visible: boolean; onClose: () => void; title?: string; children: React.ReactNode;
}) => (
  <Modal transparent visible={visible} onRequestClose={onClose} animationType="fade">
    <TouchableOpacity activeOpacity={1} onPress={onClose} style={{ flex: 1, backgroundColor: 'rgba(20,30,26,0.4)', justifyContent: 'center', alignItems: 'center' }}>
      <TouchableOpacity activeOpacity={1} onPress={() => { }} style={{ width: '88%', maxWidth: 400, borderRadius: 26, overflow: 'hidden', backgroundColor: '#F7F9F6', borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', elevation: 25 }}>
        {title && (
          <View style={{ paddingHorizontal: 20, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: '#F3F4F6', backgroundColor: '#FEF3C7' }}>
            <Text style={{ fontWeight: '900', fontSize: 16, color: '#78350F' }}>{title}</Text>
          </View>
        )}
        {children}
      </TouchableOpacity>
...
const IconPicker = ({ visible, onClose, onSelect, selected, title }: {
  visible: boolean; onClose: () => void; onSelect: (icon: string) => void; selected: string; title?: string;
}) => (
  <Modal transparent visible={visible} onRequestClose={onClose} animationType="slide">
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F7F9F6' }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: '#F3F4F6' }}>
        ...
        <Text style={{ fontSize: 17, fontWeight: '900', color: TEXT_PRIMARY }}>{title || 'Pick an Icon'}</Text>
        <View style={{ width: 42 }} />
      </View>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12, justifyContent: 'center', paddingBottom: 20 }}>
          {ICONS.map(ico => (
            <TouchableOpacity key={ico} activeOpacity={0.7} onPress={() => { onSelect(ico); onClose(); }}
              style={{ width: 58, height: 58, borderRadius: 18, alignItems: 'center', justifyContent: 'center', backgroundColor: selected === ico ? brand : '#F3F4F6' }}>
              <MaterialCommunityIcons name={ico as any} size={26} color={selected === ico ? '#fff' : TEXT_MUTED} />
            </TouchableOpacity>
          ))}
        </View>
      </ScrollView>
...
const ColorPicker = ({ visible, onClose, onSelect, selected, title, icon }: {
  visible: boolean; onClose: () => void; onSelect: (c: string) => void; selected: string; title?: string; icon?: string;
}) => (
  <Modal transparent visible={visible} onRequestClose={onClose} animationType="slide">
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F7F9F6' }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: '#F3F4F6' }}>
        ...
        <Text style={{ fontSize: 17, fontWeight: '900', color: TEXT_PRIMARY }}>{title || 'Pick a Color'}</Text>
        <View style={{ width: 42 }} />
      </View>
      <ScrollView contentContainerStyle={{ padding: 24 }} showsVerticalScrollIndicator={false}>
        <View style={{ alignItems: 'center', paddingVertical: 28, borderRadius: 24, marginBottom: 24, backgroundColor: 'rgba(255,255,255,0.92)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)' }}>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12, justifyContent: 'center', paddingBottom: 20 }}>
          {COLORS.map(c => (
            <TouchableOpacity key={c} activeOpacity={0.7} onPress={() => onSelect(c)}
              style={{ width: 64, height: 64, borderRadius: 18, alignItems: 'center', justifyContent: 'center', backgroundColor: c, borderWidth: selected === c ? 4 : 0, borderColor: '#000', transform: selected === c ? [{ scale: 1.1 }] : [] }}>
...
// ── Main Screen ──
export default function PostHomeworkScreenV2({ navigation }: Props) {
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    await Promise.all([loadBatches(), loadSubjects(), loadStudents(), loadHomeworks()]);
    setRefreshing(false);
  }, [loadBatches, loadSubjects, loadStudents, loadHomeworks]);
    ...
    <View style={{ flex: 1, backgroundColor: '#F7F9F6' }}>
      ...
      <View style={{ paddingTop: Math.max(insets.top, 20), paddingHorizontal: 20, paddingBottom: 8 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          ...
          <TouchableOpacity activeOpacity={0.9} onPress={() => {
            ...
            style={{ backgroundColor: ACCENT, width: 52, height: 52, borderRadius: 18, alignItems: 'center', justifyContent: 'center', elevation: 4, shadowColor: ACCENT, shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.35, shadowRadius: 10 }}>
            <MaterialCommunityIcons name={tab === 'post' ? 'send' : tab === 'subjects' ? 'book-plus' : 'plus'} size={26} color="white" />
          </TouchableOpacity>
        </View>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 11, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 2, color: TEXT_MUTED, marginBottom: 4 }}>
              {user?.role === 'tuition_teacher' ? 'Tuition Teacher' : 'Teacher'}
            </Text>
            <Text style={{ color: TEXT_PRIMARY, fontSize: 26, fontWeight: '700', letterSpacing: -0.5 }}>{tabTitle}</Text>
            <Text style={{ color: '#DB2777', fontSize: 13, fontWeight: '800', marginTop: 4 }}>
              {tab === 'post' ? `${homeworks.length} posted` : tab === 'subjects' ? `${subjects.length} saved` : `${batches.length} classes`}
            </Text>
          </View>
          <View style={{ backgroundColor: 'rgba(245,158,11,0.12)', width: 48, height: 48, borderRadius: 16, alignItems: 'center', justifyContent: 'center' }}>
            <Image source={require('../../../assets/icons/note-book.png')} style={{ width: 30, height: 30 }} resizeMode="contain" />
          </View>
        </View>
      </View>
      ...
      >
        ...
        <View style={{ paddingHorizontal: 20, paddingVertical: 6 }}>
          <View style={{ flexDirection: 'row', backgroundColor: 'rgba(255,255,255,0.92)', borderRadius: 18, padding: 5, borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', elevation: 3 }}>
            {tabs.map(t => {
              const active = tab === t.key;
              return (
                <TouchableOpacity key={t.key} activeOpacity={0.7} onPress={() => setTab(t.key)}
                  style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 11, borderRadius: 14, backgroundColor: active ? brand : 'transparent' }}>
                  <MaterialCommunityIcons name={t.icon} size={18} color={active ? '#fff' : TEXT_MUTED} />
                  <Text style={{ fontWeight: '900', fontSize: 12, marginLeft: 6, color: active ? '#fff' : TEXT_SECONDARY }}>{t.label}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
        ...
        {tab === 'subjects' && (
          <View style={{ paddingHorizontal: 20, paddingTop: 12 }}>
            {subjects.length === 0 ? (
              <View style={{ alignItems: 'center', paddingVertical: 48, backgroundColor: 'rgba(255,255,255,0.92)', borderRadius: 28, borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', borderStyle: 'dashed' }}>
                <MaterialCommunityIcons name="book-off" size={52} color="#C7D1CB" />
                <Text style={{ color: TEXT_MUTED, fontWeight: '800', fontSize: 16, marginTop: 16 }}>No subjects yet</Text>
              ...
              <View>
                <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12, paddingHorizontal: 2 }}>
                  <Image source={require('../../../assets/icons/exam-results.png')} style={{ width: 22, height: 22, marginRight: 8 }} resizeMode="contain" />
                  <Text style={{ fontSize: 16, fontWeight: '900', color: TEXT_PRIMARY }}>Saved Subjects</Text>
                </View>
                {subjects.map((s) => (
                  <TouchableOpacity key={s.id} activeOpacity={0.85} onPress={() => startEditSubject(s)}
                    style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 14, borderRadius: 18, marginBottom: 12, borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', backgroundColor: 'rgba(255,255,255,0.92)', elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 6 }}>
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        <Text style={{ fontSize: 12, fontWeight: '600', marginTop: 2, color: TEXT_MUTED }}>{s.batch?.name || 'No class mapped'}</Text>
                      </View>
        ...
        {tab === 'classes' && (
          <View style={{ paddingHorizontal: 20, paddingTop: 12 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12, paddingHorizontal: 4 }}>
              <Image source={require('../../../assets/icons/team.png')} style={{ width: 22, height: 22, marginRight: 8 }} resizeMode="contain" />
              <Text style={{ fontSize: 16, fontWeight: '900', color: TEXT_PRIMARY }}>All Classes</Text>
            </View>
            {batches.length === 0 ? (
              <View style={{ alignItems: 'center', paddingVertical: 48, backgroundColor: 'rgba(255,255,255,0.92)', borderRadius: 28, borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', borderStyle: 'dashed' }}>
                <MaterialCommunityIcons name="shape-outline" size={52} color="#C7D1CB" />
                <Text style={{ color: TEXT_MUTED, fontWeight: '800', fontSize: 16, marginTop: 16 }}>No classes yet</Text>
                ...
                <View key={b.id}
                  style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 14, borderRadius: 18, marginBottom: 12, borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', backgroundColor: 'rgba(255,255,255,0.92)', elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 6 }}>
                    ...
                    <TouchableOpacity style={{ flex: 1 }} activeOpacity={0.85} onPress={() => { startEditBatch(b); setShowCreateModal('batch'); }}>
                      <Text style={{ fontWeight: '800', fontSize: 14, color: TEXT_PRIMARY }}>{b.name}</Text>
                      <Text style={{ fontSize: 12, fontWeight: '600', marginTop: 2, color: TEXT_MUTED }}>{studentCount} students</Text>
                    </TouchableOpacity>
        ...
        {tab === 'post' && (
          <View style={{ paddingHorizontal: 20, paddingTop: 12 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12, paddingHorizontal: 4 }}>
              <Image source={require('../../../assets/icons/megaphone.png')} style={{ width: 22, height: 22, marginRight: 8 }} resizeMode="contain" />
              <Text style={{ fontSize: 16, fontWeight: '900', color: TEXT_PRIMARY }}>Homework Posts</Text>
            </View>
            {loadingHomeworks ? (
              ...
              <View style={{ alignItems: 'center', paddingVertical: 48, backgroundColor: 'rgba(255,255,255,0.92)', borderRadius: 28, borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', borderStyle: 'dashed' }}>
                <MaterialCommunityIcons name="book-off" size={52} color="#C7D1CB" />
                <Text style={{ color: TEXT_MUTED, fontWeight: '800', fontSize: 16, marginTop: 16 }}>No homework posted yet</Text>
                <Text style={{ color: '#B0B7C3', fontSize: 12, marginTop: 4 }}>Tap + to post your first homework</Text>
              </View>
            ) : homeworks.map(h => (
              <View key={h.id} style={{ borderRadius: 18, marginBottom: 12, borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', backgroundColor: 'rgba(255,255,255,0.92)', overflow: 'hidden', elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 6 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 13 }}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontWeight: '800', fontSize: 14, color: TEXT_PRIMARY }}>{h.title}</Text>
                    <Text style={{ fontSize: 11, fontWeight: '600', marginTop: 2, color: TEXT_MUTED }}>{h.subject?.name || 'General'}{h.batch?.name ? ` · ${h.batch.name}` : ''}</Text>
                  </View>
                    ...
                    {(h.attachments as any[]).map((a: any, idx: number) => {
                      ...
                      return (
                        <TouchableOpacity key={idx} activeOpacity={0.7} onPress={() => openAttachment({ uri: mediaUrl || a.path || a.uri, name: a.name || 'attachment', type: a.type || (isPdf ? 'application/pdf' : 'image/jpeg'), size: a.size || 0 })}
                          style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 9, borderRadius: 12, marginTop: 8, backgroundColor: 'rgba(247,249,246,0.8)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)' }}>
                          ...
                          <Text style={{ flex: 1, fontSize: 12, fontWeight: '700', color: TEXT_SECONDARY }} numberOfLines={1}>{a.name || 'attachment'}</Text>
      ...
      <Modal transparent visible={showClassPicker} onRequestClose={() => setShowClassPicker(false)} animationType="slide">
        <SafeAreaView style={{ flex: 1, backgroundColor: '#F7F9F6' }}>
          <PickerHeader title="Select Class" onClose={() => setShowClassPicker(false)}
            right={<TouchableOpacity onPress={() => { setSelectedBatch(null); setShowClassPicker(false); }} style={{ backgroundColor: 'rgba(245,158,11,0.15)', paddingHorizontal: 14, paddingVertical: 7, borderRadius: 12 }}><Text style={{ color: ACCENT, fontWeight: '900', fontSize: 11 }}>All</Text></TouchableOpacity>} />
          {batches.length === 0 ? (
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
              <MaterialCommunityIcons name="google-classroom" size={48} color="#9CA3AF" />
              <Text style={{ color: TEXT_MUTED, fontWeight: '800', fontSize: 14, marginTop: 12 }}>No classes available</Text>
            </View>
          ) : (
              ...
              {batches.map(b => {
                ...
                return (
                  <TouchableOpacity key={b.id} activeOpacity={0.7} onPress={() => { setSelectedBatch(b.id); setShowClassPicker(false); }}
                    style={{ flexDirection: 'row', alignItems: 'center', padding: 16, borderRadius: 18, marginBottom: 12, borderWidth: 1, borderColor: active ? brandDark : 'rgba(255,255,255,0.6)', backgroundColor: active ? brand : 'rgba(255,255,255,0.92)', overflow: 'hidden' }}>
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontWeight: '900', fontSize: 15, color: active ? '#fff' : TEXT_PRIMARY }}>{b.name}</Text>
                      <Text style={{ fontSize: 11, fontWeight: '700', color: active ? '#fff' : TEXT_MUTED }}>{studentCount} students</Text>
                    </View>
      ...
      <Modal transparent visible={showHomeworkStudentPicker} onRequestClose={() => setShowHomeworkStudentPicker(false)} animationType="slide">
        <SafeAreaView style={{ flex: 1, backgroundColor: '#F7F9F6' }}>
          <PickerHeader title="Assign to Students" onClose={() => setShowHomeworkStudentPicker(false)}
            ...
            right={<TouchableOpacity onPress={() => { setSelectedStudents([]); setShowHomeworkStudentPicker(false); }} style={{ backgroundColor: 'rgba(245,158,11,0.15)', paddingHorizontal: 14, paddingVertical: 7, borderRadius: 12 }}><Text style={{ color: ACCENT, fontWeight: '900', fontSize: 11 }}>Clear</Text></TouchableOpacity>} />
          {allStudents.length === 0 ? (
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 30 }}>
              <MaterialCommunityIcons name="account-off" size={48} color="#9CA3AF" />
              <Text style={{ color: TEXT_MUTED, fontWeight: '800', fontSize: 14, marginTop: 12 }}>No students available</Text>
            ...
            <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 30 }} showsVerticalScrollIndicator={false}>
              <Text style={{ fontSize: 12, fontWeight: '700', color: TEXT_MUTED, marginBottom: 12 }}>
              ...
              {allStudents.map(st => {
                ...
                return (
                  <TouchableOpacity key={st.id} activeOpacity={0.7} onPress={() => toggleStudent(st.id)}
                    style={{ flexDirection: 'row', alignItems: 'center', padding: 14, borderRadius: 18, marginBottom: 10, borderWidth: 1, borderColor: active ? brandDark : 'rgba(255,255,255,0.6)', backgroundColor: active ? brand : 'rgba(255,255,255,0.92)', overflow: 'hidden' }}>
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontWeight: '900', fontSize: 15, color: active ? '#fff' : TEXT_PRIMARY }}>{st.name}</Text>
                      <Text style={{ fontSize: 11, fontWeight: '700', color: active ? '#fff' : TEXT_MUTED }}>{batch?.name || 'No class'}</Text>
                    </View>
                    <View style={{ width: 24, height: 24, borderRadius: 9, alignItems: 'center', justifyContent: 'center', backgroundColor: active ? '#fff' : '#F3F4F6' }}>
      ...
      <Modal transparent visible={showNewBatchPicker} onRequestClose={() => setShowNewBatchPicker(false)} animationType="slide">
        <SafeAreaView style={{ flex: 1, backgroundColor: '#F7F9F6' }}>
          <PickerHeader title="Link to Class" onClose={() => setShowNewBatchPicker(false)}
            right={<TouchableOpacity onPress={() => { setNewSubjectBatchId(null); setShowNewBatchPicker(false); }} style={{ backgroundColor: 'rgba(245,158,11,0.15)', paddingHorizontal: 14, paddingVertical: 7, borderRadius: 12 }}><Text style={{ color: ACCENT, fontWeight: '900', fontSize: 11 }}>None</Text></TouchableOpacity>} />
          <ScrollView contentContainerStyle={{ padding: 20 }} showsVerticalScrollIndicator={false}>
            {batches.length === 0 ? (
              ...
              return (
                <TouchableOpacity key={b.id} activeOpacity={0.7} onPress={() => { setNewSubjectBatchId(b.id); setShowNewBatchPicker(false); }}
                  ...
                  <Text style={{ flex: 1, fontWeight: '900', fontSize: 15, color: active ? '#fff' : TEXT_PRIMARY }}>{b.name}</Text>
                  {active && <MaterialCommunityIcons name="check-circle" size={22} color="#fff" />}
                </TouchableOpacity>
              );
            })}
      ...
      <Modal transparent visible={showSubjectPicker} onRequestClose={() => setShowSubjectPicker(false)} animationType="slide">
        <SafeAreaView style={{ flex: 1, backgroundColor: '#F7F9F6' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: '#F3F4F6' }}>
            <TouchableOpacity onPress={() => setShowSubjectPicker(false)} style={{ width: 42, height: 42, borderRadius: 14, backgroundColor: 'rgba(255,255,255,0.92)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', alignItems: 'center', justifyContent: 'center' }}>
              <MaterialCommunityIcons name="close" size={22} color={TEXT_PRIMARY} />
            </TouchableOpacity>
            <Text style={{ fontSize: 17, fontWeight: '900', color: TEXT_PRIMARY }}>Pick a Subject</Text>
            <TouchableOpacity onPress={() => { setSelectedSubjectId(null); setShowSubjectPicker(false); }} style={{ paddingHorizontal: 14, paddingVertical: 7, borderRadius: 12, backgroundColor: 'rgba(245,158,11,0.15)' }}>
              <Text style={{ color: ACCENT, fontWeight: '900', fontSize: 11 }}>ALL</Text>
            </TouchableOpacity>
          </View>
          {subjects.length === 0 ? (
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
              <MaterialCommunityIcons name="book-off" size={48} color="#9CA3AF" />
              <Text style={{ color: TEXT_MUTED, fontWeight: '800', fontSize: 14, marginTop: 12 }}>No subjects yet</Text>
              <Text style={{ color: '#B0B7C3', fontSize: 12, marginTop: 4 }}>Create one in Subjects tab</Text>
            </View>
          ) : (
            <ScrollView contentContainerStyle={{ padding: 16 }} showsVerticalScrollIndicator={false}>
              <Text style={{ fontSize: 11, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12, paddingHorizontal: 4, color: TEXT_MUTED }}>{subjects.length} subjects</Text>
              {subjects.map(s => {
                ...
                return (
                  <TouchableOpacity key={s.id} activeOpacity={0.85} onPress={() => { setSelectedSubjectId(s.id); setShowSubjectPicker(false); }}
                    style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14, borderRadius: 18, marginBottom: 12, borderWidth: 1, borderColor: active ? brandDark : 'rgba(255,255,255,0.6)', backgroundColor: active ? brand : 'rgba(255,255,255,0.92)', overflow: 'hidden', elevation: active ? 4 : 1, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 6 }}>
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontWeight: '800', fontSize: 15, color: active ? '#fff' : TEXT_PRIMARY }} numberOfLines={1}>{s.name}</Text>
                      {s.batch ? (
                        ...
                        <Text style={{ fontSize: 11, fontWeight: '600', marginTop: 4, color: active ? 'rgba(255,255,255,0.85)' : TEXT_MUTED }}>No class mapped</Text>
                      )}
                    </View>
                    {active ? (
      ...
      <PickerModal visible={showStudentPicker} onClose={() => setShowStudentPicker(false)} title={`Add to ${selectedManageBatch?.name || ''}`}>
        {unassignedStudents.length === 0 ? (
          <View style={{ paddingHorizontal: 20, paddingVertical: 32, alignItems: 'center' }}>
            ...
            <Text style={{ color: TEXT_MUTED, fontWeight: '700', fontSize: 14, marginTop: 12 }}>All students assigned</Text>
          </View>
        ) : (
          <ScrollView bounces={false} style={{ maxHeight: 350 }}>
            {unassignedStudents.map(st => (
              <TouchableOpacity key={st.id} activeOpacity={0.7} onPress={() => { assignStudentToBatch(st.id); setShowStudentPicker(false); }}
                ...
                <View style={{ flex: 1 }}>
                  <Text style={{ fontWeight: '700', fontSize: 14, color: TEXT_SECONDARY }}>{st.name}</Text>
                  <Text style={{ fontSize: 10, fontWeight: '700', color: TEXT_MUTED }}>ID: {st.student_id || st.studentId || st.id}</Text>
                </View>
      ...
      {/* Icon Pickers */}
      <IconPicker visible={showSubjectIconPicker} onClose={() => setShowSubjectIconPicker(false)} onSelect={setNewSubjectIcon} selected={newSubjectIcon} title="Pick Subject Icon" />
      <IconPicker visible={showBatchIconPicker} onClose={() => setShowBatchIconPicker(false)} onSelect={setNewBatchIcon} selected={newBatchIcon} title="Pick Class Icon" />
      <ColorPicker visible={showSubjectColorPicker} onClose={() => setShowSubjectColorPicker(false)} onSelect={setNewSubjectColor} selected={newSubjectColor} title="Subject Color" icon={newSubjectIcon} />
      <ColorPicker visible={showBatchColorPicker} onClose={() => setShowBatchColorPicker(false)} onSelect={setNewBatchColor} selected={newBatchColor} title="Class Color" icon={newBatchIcon} />
          ...
          <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 30 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" {...(Platform.OS === 'ios' ? { automaticallyAdjustKeyboardInsets: true } : {})}>
            <Card>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
                <View style={{ width: 44, height: 44, borderRadius: 14, backgroundColor: 'rgba(245,158,11,0.12)', alignItems: 'center', justifyContent: 'center', marginRight: 12 }}>
                  <Image source={require('../../../assets/icons/team.png')} style={{ width: 26, height: 26 }} resizeMode="contain" />
                </View>
                <View>
                  <Text style={{ fontWeight: '900', fontSize: 15, color: TEXT_PRIMARY }}>Class & Subject</Text>
                  <Text style={{ fontSize: 11, fontWeight: '600', marginTop: 1, color: TEXT_MUTED }}>Select where to assign this homework</Text>
                </View>
              </View>
              {isTuitionTeacher && (
                <TouchableOpacity activeOpacity={0.7} onPress={() => setShowClassPicker(true)}
                  style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 14, borderRadius: 16, marginBottom: 12, borderWidth: 1, borderColor: selectedBatch ? brandDark : 'rgba(255,255,255,0.6)', backgroundColor: selectedBatch ? brand : 'rgba(255,255,255,0.92)', overflow: 'hidden' }}>
                  <View style={{ width: 38, height: 38, borderRadius: 13, alignItems: 'center', justifyContent: 'center', marginRight: 12, backgroundColor: selectedBatch ? 'rgba(255,255,255,0.25)' : 'rgba(245,158,11,0.12)' }}>
                    <MaterialCommunityIcons name="google-classroom" size={18} color={selectedBatch ? '#fff' : brand} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontWeight: '800', fontSize: 14, color: selectedBatch ? '#fff' : TEXT_PRIMARY }} numberOfLines={1}>
              ...
              <TouchableOpacity activeOpacity={0.7} onPress={() => setShowSubjectPicker(true)}
                style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 14, borderRadius: 16, borderWidth: 1, borderColor: selectedSubjectId ? brandDark : 'rgba(255,255,255,0.6)', backgroundColor: selectedSubjectId ? brand : 'rgba(255,255,255,0.92)', overflow: 'hidden' }}>
                <View style={{ width: 38, height: 38, borderRadius: 13, alignItems: 'center', justifyContent: 'center', marginRight: 12, backgroundColor: selectedSubjectId ? 'rgba(255,255,255,0.25)' : 'rgba(245,158,11,0.12)' }}>
                  <MaterialCommunityIcons name="book-education" size={18} color={selectedSubjectId ? '#fff' : brand} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontWeight: '800', fontSize: 14, color: selectedSubjectId ? '#fff' : TEXT_PRIMARY }} numberOfLines={1}>
              ...
              <TouchableOpacity activeOpacity={0.7} onPress={() => setShowHomeworkStudentPicker(true)}
                style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 14, borderRadius: 16, marginTop: 14, borderWidth: 1, borderColor: selectedStudents.length > 0 ? brandDark : 'rgba(255,255,255,0.6)', backgroundColor: selectedStudents.length > 0 ? brand : 'rgba(255,255,255,0.92)', overflow: 'hidden' }}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={{ fontWeight: '800', fontSize: 14, color: selectedStudents.length > 0 ? '#fff' : TEXT_PRIMARY }}>
                ...
                <MaterialCommunityIcons name="chevron-down" size={20} color={selectedStudents.length > 0 ? '#fff' : TEXT_MUTED} />
              </TouchableOpacity>
            </Card>

            <Card>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
                <View style={{ width: 44, height: 44, borderRadius: 14, backgroundColor: 'rgba(139,92,246,0.12)', alignItems: 'center', justifyContent: 'center', marginRight: 12 }}>
                  <Image source={require('../../../assets/icons/note-book.png')} style={{ width: 26, height: 26 }} resizeMode="contain" />
                </View>
                <View>
                  <Text style={{ fontWeight: '900', fontSize: 15, color: TEXT_PRIMARY }}>Homework Details</Text>
                  <Text style={{ fontSize: 11, fontWeight: '600', marginTop: 1, color: TEXT_MUTED }}>Title, description & due date</Text>
                </View>
              </View>
              <SurveyTag label="Title *" icon="format-text" value={title} onChange={setTitle} placeholder="e.g. Chapter 5: Algebra Basics" />
              <SurveyTag label="DESCRIPTION *" icon="text-long" value={description} onChange={setDescription} placeholder="Describe the assignment in detail..." multiline />
              <SurveyTag label="DUE DATE *" icon="calendar" value={dueDate} onChange={setDueDate} placeholder="e.g. 2026-08-15 or Tomorrow" />
            </Card>

            <Card>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                <View style={{ width: 44, height: 44, borderRadius: 14, backgroundColor: 'rgba(59,130,246,0.12)', alignItems: 'center', justifyContent: 'center', marginRight: 12 }}>
                  <Image source={require('../../../assets/icons/database.png')} style={{ width: 26, height: 26 }} resizeMode="contain" />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontWeight: '900', fontSize: 15, color: TEXT_PRIMARY }}>Attachments</Text>
                  <Text style={{ fontSize: 11, fontWeight: '600', marginTop: 1, color: TEXT_MUTED }}>{attachments.length > 0 ? `${attachments.length} file(s) selected` : 'Add images or PDFs'}</Text>
                </View>
              ...
              {attachments.length > 0 ? (
                attachments.map((a, idx) => (
                  <View key={idx} style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 12, paddingHorizontal: 14, borderRadius: 16, marginTop: 8, backgroundColor: 'rgba(247,249,246,0.8)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)' }}>
                    ...
                    <Text style={{ flex: 1, fontSize: 12, fontWeight: '800', color: TEXT_SECONDARY }} numberOfLines={1}>{a.name}</Text>
                    <Text style={{ fontSize: 10, fontWeight: '700', marginRight: 10, color: TEXT_MUTED }}>{formatSize(a.size)}</Text>
                ...
                <View style={{ alignItems: 'center', paddingVertical: 24 }}>
                  <MaterialCommunityIcons name="cloud-upload-outline" size={36} color="#C7D1CB" />
                  <Text style={{ fontSize: 12, fontWeight: '600', marginTop: 6, color: TEXT_MUTED }}>Tap the icons above to attach files</Text>
                </View>
              )}
            </Card>
      ...
      <Modal transparent visible={showCreateModal === 'subject'} onRequestClose={() => setShowCreateModal(null)} animationType="slide">
        <SafeAreaView style={{ flex: 1, backgroundColor: '#F7F9F6' }}>
          ...
          <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 30 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" {...(Platform.OS === 'ios' ? { automaticallyAdjustKeyboardInsets: true } : {})}>
            <Card>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
                <View style={{ width: 44, height: 44, borderRadius: 14, alignItems: 'center', justifyContent: 'center', marginRight: 12, backgroundColor: newSubjectColor }}>
                  <MaterialCommunityIcons name={newSubjectIcon as any} size={20} color="white" />
                </View>
                <View>
                  <Text style={{ fontSize: 16, fontWeight: '900', color: TEXT_PRIMARY }}>{editingSubject ? 'Edit Subject' : 'New Subject'}</Text>
                  <Text style={{ fontSize: 12, fontWeight: '600', color: TEXT_MUTED }}>{editingSubject ? 'Update subject details' : 'Map it to a class for quick posting'}</Text>
                </View>
              </View>
              <View style={{ height: 1, backgroundColor: '#F3F4F6', marginBottom: 16 }} />
              <Survey label="Subject Name" icon="book" value={newSubjectName} onChange={setNewSubjectName} placeholder="e.g. Mathematics" />
              {user?.role === 'master_admin' ? (
                <View style={{ marginBottom: 18 }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                    <MaterialCommunityIcons name="office-building" size={16} color={brand} />
                    <Text style={{ fontSize: 12, fontWeight: '800', marginLeft: 8, textTransform: 'uppercase', letterSpacing: 1, color: '#B45309' }}>Branch</Text>
                  </View>
                  <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                    {branches.length === 0 ? (
                      <Text style={{ fontSize: 12, fontWeight: '600', color: TEXT_MUTED, paddingVertical: 10 }}>No branches available yet. Create one in Branch Management first.</Text>
                    ) : branches.map((br: any) => {
                      ...
                      return (
                        <TouchableOpacity key={br.id} activeOpacity={0.8} onPress={() => setNewSubjectBranchId(Number(br.id))}
                          style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 10, borderRadius: 14, marginRight: 8, borderWidth: 1, borderColor: active ? brand : '#E5E7EB', backgroundColor: active ? brand : 'rgba(255,255,255,0.92)' }}>
                          <MaterialCommunityIcons name={active ? 'check-circle' : 'office-building'} size={16} color={active ? '#fff' : TEXT_MUTED} style={{ marginRight: 6 }} />
                          <Text style={{ fontWeight: '800', fontSize: 13, color: active ? '#fff' : TEXT_SECONDARY }}>{br.name}</Text>
                        </TouchableOpacity>
                      );
                    })}
                  </ScrollView>
              ...
              <View style={{ marginBottom: 18 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  <Text style={{ fontSize: 12, fontWeight: '800', marginLeft: 8, textTransform: 'uppercase', letterSpacing: 1, color: '#B45309' }}>Link to Class</Text>
                </View>
                <TouchableOpacity activeOpacity={0.7} onPress={() => setShowNewBatchPicker(true)}
                  style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.92)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', borderRadius: 16, paddingHorizontal: 16, paddingVertical: 14 }}>
                  <MaterialCommunityIcons name="google-classroom" size={18} color={brand} />
                  <Text style={{ flex: 1, fontWeight: '700', fontSize: 13, marginHorizontal: 10, color: newSubjectBatchId ? TEXT_PRIMARY : TEXT_MUTED }}>
              ...
              <View style={{ marginBottom: 18 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  <Text style={{ fontSize: 12, fontWeight: '800', marginLeft: 8, textTransform: 'uppercase', letterSpacing: 1, color: '#B45309' }}>Icon & Color</Text>
                </View>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                  ...
                  <TouchableOpacity activeOpacity={0.7} onPress={() => setShowSubjectColorPicker(true)}
                    style={{ flex: 1, flexDirection: 'row', alignItems: 'center', paddingVertical: 14, paddingHorizontal: 16, borderRadius: 16, borderWidth: 1, borderColor: '#E5E7EB', borderStyle: 'dashed', backgroundColor: 'rgba(255,255,255,0.92)' }}>
                    ...
                    <Text style={{ flex: 1, fontWeight: '800', fontSize: 13, color: TEXT_MUTED }}>Pick Color</Text>
                    <MaterialCommunityIcons name="chevron-down" size={18} color="#9CA3AF" />
                  </TouchableOpacity>
                </View>
              </View>
      ...
      <Modal transparent visible={showCreateModal === 'batch'} onRequestClose={() => setShowCreateModal(null)} animationType="slide">
        <SafeAreaView style={{ flex: 1, backgroundColor: '#F7F9F6' }}>
          ...
          <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 30 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" {...(Platform.OS === 'ios' ? { automaticallyAdjustKeyboardInsets: true } : {})}>
            <Card>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
                <View style={{ width: 44, height: 44, borderRadius: 14, alignItems: 'center', justifyContent: 'center', marginRight: 12, backgroundColor: newBatchColor }}>
                  <MaterialCommunityIcons name={newBatchIcon as any} size={20} color="white" />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 16, fontWeight: '900', color: TEXT_PRIMARY }}>{editingBatch ? 'Edit Class' : 'New Class'}</Text>
                  <Text style={{ fontSize: 12, fontWeight: '600', color: TEXT_MUTED }}>{editingBatch ? 'Update class details' : 'Create a batch and add students'}</Text>
                </View>
                {editingBatch && (
              ...
              <View style={{ height: 1, backgroundColor: '#F3F4F6', marginBottom: 16 }} />
              <Survey label="Class Name" icon="google-classroom" value={newBatchName} onChange={setNewBatchName} placeholder="e.g. Morning Batch A" />
              <Survey label="Description" icon="text-long" value={newBatchDesc} onChange={setNewBatchDesc} placeholder="Optional description" />
              {user?.role === 'master_admin' ? (
                <View style={{ marginBottom: 18 }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                    <MaterialCommunityIcons name="office-building" size={16} color={brand} />
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: '#F7F9F6' }}>
          ...
          <View style={{ flex: 1, paddingHorizontal: 20, paddingTop: 16 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 14 }}>
              ...
              <Text style={{ fontSize: 16, fontWeight: '900', marginLeft: 8, color: TEXT_PRIMARY }}>Students ({batchStudents.length})</Text>
            </View>
            {batchStudents.length === 0 ? (
              <View style={{ alignItems: 'center', paddingVertical: 44, backgroundColor: 'rgba(255,255,255,0.92)', borderRadius: 24, borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', borderStyle: 'dashed' }}>
                <MaterialCommunityIcons name="account-off" size={48} color="#D1D5DB" />
                <Text style={{ color: TEXT_MUTED, fontWeight: '700', fontSize: 14, marginTop: 12 }}>No students in this class</Text>
                <Text style={{ color: '#B0B7C3', fontSize: 12, marginTop: 4 }}>Tap + to add students</Text>
              </View>
            ) : (
              <ScrollView showsVerticalScrollIndicator={false}>
                {batchStudents.map(st => (
                  <View key={st.id} style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 12, borderRadius: 16, marginBottom: 10, backgroundColor: 'rgba(255,255,255,0.92)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)' }}>
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontWeight: '800', fontSize: 14, color: TEXT_PRIMARY }}>{st.name}</Text>
                      <Text style={{ fontSize: 10, fontWeight: '700', color: TEXT_MUTED }}>ID: {st.student_id || st.studentId || st.id}</Text>
                    </View>
