...
import { View, Text, ScrollView, TouchableOpacity, ActivityIndicator, RefreshControl, Alert, Modal, TextInput, FlatList, Dimensions, Animated as RNAnimated, NativeSyntheticEvent, NativeScrollEvent, Linking, Image, StyleSheet } from 'react-native';
...
export default function HomeworkScreenV2({ navigation }: Props) {
          ...
          return (
            <TouchableOpacity key={idx}
              ...
              <View style={{ flex: 1, marginLeft: 12 }}>
                <Text style={{ fontSize: 12, fontWeight: '700', color: '#374151' }} numberOfLines={1}>{name}</Text>
                {d.file_size && <Text style={{ fontSize: 9, fontWeight: '600', color: '#9CA3AF', marginTop: 1 }}>{formatSize(d.file_size)}</Text>}
              </View>
      ...
      <View style={{ width: CARD_WIDTH, paddingRight: 0 }}>
        <View style={{
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 20 }}>
            <View style={{ paddingHorizontal: 20, paddingTop: 18, paddingBottom: 14, borderBottomWidth: 1, borderBottomColor: '#F9FAFB' }}>
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                <View style={{ marginLeft: 12, flex: 1 }}>
                  <Text style={{ fontSize: 14, fontWeight: '900', color: '#111' }}>{h.teacher?.name || 'Teacher'}</Text>
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 2 }}>
                    ...
                    <Text style={{ fontSize: 9, fontWeight: '600', color: '#9CA3AF', marginLeft: 8 }}>#{h.id}</Text>
                  </View>
                </View>
                {sub && (
            ...
            <View style={{ paddingHorizontal: 20, paddingTop: 16 }}>
              <Text style={{ fontSize: 20, fontWeight: '900', letterSpacing: -0.3, lineHeight: 26, color: '#111' }}>
                {h.title}
              </Text>
              {h.description ? (
                <Text style={{ fontSize: 13, fontWeight: '500', lineHeight: 20, color: '#6B7280', marginTop: 10 }}>
                  {h.description}
                </Text>
              ) : null}
            </View>
            ...
            <View style={{ paddingHorizontal: 20, marginTop: 16 }}>
              {sub ? (
                <View style={{ backgroundColor: '#DCFCE7', borderRadius: 16, padding: 16, borderWidth: 1, borderColor: '#A7F3D0' }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center' }}>
    ...
    return (
      <Modal transparent visible={!!submitModal} onRequestClose={() => { setSubmitModal(null); setSubmitFiles([]); setComment(''); }} animationType="slide">
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.4)' }}>
          ...
          <View style={{ backgroundColor: '#FFFFFF', borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: '75%', paddingBottom: 40 }}>
            <View style={{ paddingHorizontal: 24, paddingTop: 16, paddingBottom: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: '#F3F4F6' }}>
              <Text style={{ fontSize: 18, fontWeight: '900', color: '#111', flex: 1 }}>Submit Homework</Text>
            ...
            <ScrollView style={{ padding: 24 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                ...
                <Text style={{ fontSize: 9, fontWeight: '600', color: '#9CA3AF', marginLeft: 8 }}>#{submitModal.id}</Text>
              </View>
              <Text style={{ fontSize: 17, fontWeight: '900', color: '#111' }}>{submitModal.title}</Text>
              {submitModal.description ? <Text style={{ fontSize: 13, fontWeight: '500', color: '#6B7280', marginTop: 8 }}>{submitModal.description}</Text> : null}
              ...
              {sub ? (
                ...
                <>
                  <View style={{ marginTop: 16 }}>
                    <Text style={{ fontSize: 11, fontWeight: '700', color: '#6B7280', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>Attachments</Text>
                    {submitFiles.map((f, i) => (
                      <View key={i} style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(245,158,11,0.08)', borderRadius: 12, padding: 10, marginBottom: 6 }}>
                        <MaterialCommunityIcons name="file" size={18} color="#F59E0B" />
                        <Text style={{ flex: 1, fontSize: 12, fontWeight: '700', color: '#374151', marginLeft: 8 }} numberOfLines={1}>{f.name}</Text>
                        <Text style={{ fontSize: 9, fontWeight: '700', color: '#9CA3AF', marginRight: 8 }}>{formatSize(f.size)}</Text>
                    ...
                    <View style={{ flexDirection: 'row', gap: 10, marginTop: 8 }}>
                      <TouchableOpacity onPress={pickSubmitImage} style={{ flex: 1, backgroundColor: '#F3F4F6', borderRadius: 14, paddingVertical: 14, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', borderWidth: 1, borderColor: '#E5E7EB' }}>
                      ...
                      <TouchableOpacity onPress={pickSubmitDoc} style={{ flex: 1, backgroundColor: '#F3F4F6', borderRadius: 14, paddingVertical: 14, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', borderWidth: 1, borderColor: '#E5E7EB' }}>
                  ...
                  <View style={{ marginTop: 16 }}>
                    <Text style={{ fontSize: 11, fontWeight: '700', color: '#6B7280', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>Comment (optional)</Text>
                    <View style={{ backgroundColor: 'rgba(245,158,11,0.05)', borderRadius: 14, borderWidth: 1, borderColor: 'rgba(245,158,11,0.2)', paddingHorizontal: 14, paddingTop: 12 }}>
                      <TextInput value={comment} onChangeText={setComment} placeholder="Add a note..." placeholderTextColor="#9CA3AF" multiline numberOfLines={3} textAlignVertical="top" style={{ fontWeight: '600', fontSize: 14, lineHeight: 20, paddingBottom: 12, color: '#374151' }} />
                    </View>
                  </View>
    ...
    return (
      ...
      >
        <ScrollView
          ...
          maximumZoomScale={5}
          bouncesZoom
          contentContainerStyle={{ flex: 1, alignItems: 'center', justifyContent: 'center', minHeight: SCREEN_HEIGHT * 0.85 }}
        >
          <Image
            source={{ uri }}
            style={{ width: SCREEN_WIDTH - 40, height: SCREEN_HEIGHT * 0.8, borderRadius: 8 }}
            resizeMode="contain"
          />
        </ScrollView>
      </TouchableOpacity>
          ...
          <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.85)' }}>
            ...
            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 32 }}>
              <View style={{ backgroundColor: '#FFFFFF', borderRadius: 24, padding: 32, alignItems: 'center', width: '100%' }}>
                ...
                <Text style={{ fontWeight: '900', fontSize: 16, color: '#111', textAlign: 'center' }} numberOfLines={2}>{name}</Text>
                {viewItem.file_size && <Text style={{ fontWeight: '600', fontSize: 12, color: '#9CA3AF', marginTop: 6 }}>{formatSize(viewItem.file_size)}</Text>}
      ...
      >
        <View style={{ paddingTop: Math.max(insets.top, 20), paddingHorizontal: 20 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            <View>
              <Text style={{ color: TEXT_PRIMARY, fontSize: 26, fontWeight: '700', letterSpacing: -0.5 }}>Homework</Text>
              <Text style={{ color: '#DB2777', fontSize: 14, fontWeight: '800' }}>Assignments & Submissions</Text>
            </View>
          </View>
          {homeworks.length > 1 && (
              ...
              {homeworks.map((_, idx) => (
                <TouchableOpacity key={idx} onPress={() => flatRef.current?.scrollToIndex({ index: idx, animated: true })}>
                  <View style={{
                    ...
                    backgroundColor: currentIndex === idx ? ACCENT : '#D1D5DB',
                  }} />
                </TouchableOpacity>
              ))}
              <Text style={{ fontSize: 11, fontWeight: '700', color: TEXT_MUTED, marginLeft: 8 }}>
        ...
        {homeworks.length === 0 ? (
          <View style={{ alignItems: 'center', justifyContent: 'center', paddingHorizontal: 24, paddingTop: 32 }}>
            <View style={{ backgroundColor: 'rgba(255,255,255,0.92)', borderRadius: 24, padding: 48, alignItems: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', width: '100%' }}>
              <View style={{ backgroundColor: 'rgba(245,158,11,0.12)', width: 72, height: 72, borderRadius: 22, alignItems: 'center', justifyContent: 'center' }}>
                <Image source={require('../../../assets/icons/note-book.png')} style={{ width: 40, height: 40 }} resizeMode="contain" />
              </View>
              <Text style={{ fontWeight: '800', fontSize: 16, color: TEXT_SECONDARY, marginTop: 16 }}>No homework assigned</Text>
              <Text style={{ fontWeight: '500', fontSize: 13, color: TEXT_MUTED, marginTop: 4 }}>Check back later</Text>
            </View>
          </View>
        ) : (
          <FlatList
