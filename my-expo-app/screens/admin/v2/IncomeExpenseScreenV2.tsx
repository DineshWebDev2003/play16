...
import {
  ...
  Alert, Platform, ActivityIndicator, Image, StyleSheet, Dimensions
} from 'react-native';
...
function DatePicker({ label, value, onChange }: {
  ...
  return (
    <View style={{ flex: 1 }}>
      <Text style={{ fontSize: 9, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 1, color: TEXT_MUTED, marginBottom: 6 }}>{label}</Text>
      <TouchableOpacity onPress={() => setShow(true)} activeOpacity={0.8}
        style={{ backgroundColor: 'rgba(247,249,246,0.9)', borderRadius: 16, paddingVertical: 12, paddingHorizontal: 14, borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        <Text style={{ fontWeight: '800', fontSize: 12, color: value ? TEXT_PRIMARY : '#A0AEC0' }}>{value || 'Select'}</Text>
        <MaterialCommunityIcons name="calendar-edit" size={14} color="#F59E0B" />
      </TouchableOpacity>
      {show && <DateTimePicker value={dateValue} mode="date" display="default" accentColor="#F59E0B" onChange={(_, d) => { setShow(Platform.OS === 'ios'); if (d) { const y = d.getFullYear(); const m = String(d.getMonth() + 1).padStart(2, '0'); const day = String(d.getDate()).padStart(2, '0'); onChange(`${y}-${m}-${day}`); } }} />}
    </View>
  );
}
...
const TxItem = memo(({ item, onDelete, onEdit, isSelectMode, isSelected, onToggleSelect, isMasterAdmin, onApprove, onReject }: {
    ...
    const tint = isIncome ? '#10B9811F' : '#EF44441F';
    const iconSrc = isIncome
      ? require('../../../assets/icons/maths.png')
    ...
    <TouchableOpacity onPress={() => { if (isSelectMode) { onToggleSelect?.(item.id); } else { onEdit(item); } }} activeOpacity={0.9}
      style={{ backgroundColor: 'rgba(255,255,255,0.92)', borderRadius: BORDER_RADIUS, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: isSelected ? '#EF4444' : 'rgba(255,255,255,0.6)' }}>
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        {isSelectMode && (
          <TouchableOpacity onPress={() => onToggleSelect?.(item.id)} style={{ marginRight: 10 }}>
            <MaterialCommunityIcons name={isSelected ? 'checkbox-marked-circle' : 'checkbox-blank-circle-outline'} size={22} color={isSelected ? '#EF4444' : '#9CA3AF'} />
          </TouchableOpacity>
        )}
        <View style={{ width: 48, height: 48, borderRadius: 16, backgroundColor: tint, alignItems: 'center', justifyContent: 'center', marginRight: 12 }}>
          <Image source={iconSrc} style={{ width: 34, height: 34 }} resizeMode="contain" />
        </View>
        <View style={{ flex: 1 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            <Text style={{ fontWeight: '800', fontSize: 14, color: TEXT_PRIMARY, flex: 1 }} numberOfLines={1}>{title}</Text>
            <Text style={{ fontWeight: '900', fontSize: 15, color: accent, marginLeft: 8 }}>₹{item.amount.toLocaleString()}</Text>
          </View>
          <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 5, flexWrap: 'wrap', gap: 4 }}>
            <View style={{ backgroundColor: isIncome ? '#D1FAE5' : '#FEE2E2', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 6 }}>
              <Text style={{ fontSize: 9, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 0.5, color: isIncome ? '#065F46' : '#991B1B' }}>{item.category}</Text>
            </View>
            {feeTag && (
            ...
            {item.branch?.name && (
              <View style={{ backgroundColor: 'rgba(247,249,246,0.9)', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 6 }}>
          ...
          {!isSelectMode && (
            <View style={{ flexDirection: 'row', marginTop: 8, gap: 4 }}>
              {isMasterAdmin && isPending && (
                <>
                  <TouchableOpacity onPress={() => onApprove?.(item.id)} style={{ width: 30, height: 30, borderRadius: 10, alignItems: 'center', justifyContent: 'center', backgroundColor: '#D1FAE5' }}>
                    <MaterialCommunityIcons name="check-bold" size={13} color="#065F46" />
                  </TouchableOpacity>
                  <TouchableOpacity onPress={() => onReject?.(item.id)} style={{ width: 30, height: 30, borderRadius: 10, alignItems: 'center', justifyContent: 'center', backgroundColor: '#FEE2E2' }}>
                    <MaterialCommunityIcons name="close" size={14} color="#991B1B" />
                  </TouchableOpacity>
                </>
              )}
              <TouchableOpacity onPress={() => onEdit(item)} style={{ width: 30, height: 30, borderRadius: 10, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(247,249,246,0.9)' }}>
                <MaterialCommunityIcons name="pencil-outline" size={13} color="#6B7280" />
              </TouchableOpacity>
              <TouchableOpacity onPress={() => onDelete(item.id)} style={{ width: 30, height: 30, borderRadius: 10, alignItems: 'center', justifyContent: 'center', backgroundColor: '#FEF2F2' }}>
...
export default function IncomeExpenseScreenV2({ navigation }: Props) {
  ...
  useEffect(() => {
    if (!branchFilterId && branches.length > 0) {
      setBranchFilterId(branches[0].id?.toString() || null);
    }
  ...
  const showShare = !!branchFilterId;
  ...
  const renderDashboard = () => (
    <View style={{ marginTop: SECTION_GAP }}>
      ...
      >
        ...
        >
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <View style={{ width: 40, height: 40, borderRadius: 14, backgroundColor: 'rgba(255,255,255,0.15)', alignItems: 'center', justifyContent: 'center', marginRight: 10 }}>
                <Image source={require('../../../assets/icons/wallet.png')} style={{ width: 30, height: 30 }} resizeMode="contain" />
              </View>
              <View>
                <Text style={{ color: 'white', fontSize: 16, fontWeight: '900', letterSpacing: -0.5 }}>Treasury</Text>
                <Text style={{ color: 'rgba(255,255,255,0.7)', fontSize: 8, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 1.5, marginTop: 2 }}>Finance Ledger</Text>
              </View>
            </View>
          ...
          <View style={{ backgroundColor: 'rgba(255,255,255,0.08)', borderRadius: 14, padding: 12 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              {[
                { label: 'Income', value: rupee(totalIncome), color: '#6EE7B7' },
                { label: 'Expense', value: rupee(totalExpense), color: '#FCA5A5' },
                { label: 'Net', value: rupee(net), color: net >= 0 ? '#6EE7B7' : '#FCA5A5' },
              ].map((item, i) => (
                <View key={item.label} style={{ alignItems: 'center', flex: 1, borderRightWidth: i < 2 ? 1 : 0, borderRightColor: 'rgba(255,255,255,0.1)' }}>
                  ...
                  <Text style={{ color: 'rgba(255,255,255,0.6)', fontSize: 7, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 1, marginTop: 3 }}>{item.label}</Text>
                </View>
              ))}
            </View>
          </View>
      ...
      {/* Revenue share */}
      {showShare && (
        <View style={{ borderRadius: BORDER_RADIUS, overflow: 'hidden', marginTop: 12 }}>
          <LinearGradient colors={VIOLET} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={{ padding: 16 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                <View style={{ width: 40, height: 40, borderRadius: 14, backgroundColor: 'rgba(255,255,255,0.15)', alignItems: 'center', justifyContent: 'center', marginRight: 10 }}>
                  <Image source={require('../../../assets/icons/team.png')} style={{ width: 30, height: 30 }} resizeMode="contain" />
                </View>
                <View>
                  <Text style={{ color: 'white', fontSize: 15, fontWeight: '900', letterSpacing: -0.4 }}>Revenue Share</Text>
                  <Text style={{ color: 'rgba(255,255,255,0.7)', fontSize: 8, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 1.2, marginTop: 2 }}>{selectedBranch?.name || 'Branch'}</Text>
                </View>
              </View>
            </View>
            <View style={{ backgroundColor: 'rgba(255,255,255,0.08)', borderRadius: 14, padding: 12 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <View style={{ alignItems: 'center', flex: 1, borderRightWidth: 1, borderRightColor: 'rgba(255,255,255,0.1)' }}>
                  ...
                  <Text style={{ color: 'rgba(255,255,255,0.6)', fontSize: 7, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 1, marginTop: 3 }}>School ({sharePct}%)</Text>
                </View>
                <View style={{ alignItems: 'center', flex: 1, borderRightWidth: 1, borderRightColor: 'rgba(255,255,255,0.1)' }}>
                  <Text style={{ color: 'white', fontSize: 15, fontWeight: '900' }} numberOfLines={1}>{rupee(sharedMasterAmount)}</Text>
                  <Text style={{ color: 'rgba(255,255,255,0.6)', fontSize: 7, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 1, marginTop: 3 }}>Master ({100 - sharePct}%)</Text>
                </View>
                <View style={{ alignItems: 'center', flex: 1 }}>
                  <Text style={{ color: '#FDE68A', fontSize: 15, fontWeight: '900' }} numberOfLines={1}>{rupee(masterShareAmount)}</Text>
                  <Text style={{ color: 'rgba(253,230,138,0.8)', fontSize: 7, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 1, marginTop: 3 }}>Master Total</Text>
                </View>
              </View>
            </View>
            <Text style={{ color: 'rgba(255,255,255,0.7)', fontSize: 8, fontWeight: '700', marginTop: 10 }}>
      ...
      <View style={{ marginTop: 28 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <Text style={{ fontSize: 18, fontWeight: '600', color: TEXT_PRIMARY }}>Timeline</Text>
          <TouchableOpacity onPress={handlePrint} disabled={pdfLoading} activeOpacity={0.8}
            style={{ backgroundColor: 'rgba(255,255,255,0.92)', borderRadius: 14, paddingVertical: 9, paddingHorizontal: 14, borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', flexDirection: 'row', alignItems: 'center' }}>
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginBottom: 12 }}>
          {([
            { key: 'today' as TimelinePreset, label: 'Today' },
            { key: 'week' as TimelinePreset, label: '7 Days' },
            { key: 'month' as TimelinePreset, label: 'This Month' },
            { key: 'all' as TimelinePreset, label: 'All' },
          ]).map(p => {
            const active = activePreset === p.key;
            return (
              <TouchableOpacity key={p.key} onPress={() => applyPreset(p.key)} activeOpacity={0.9}
                style={{ flex: 1, backgroundColor: active ? '#F59E0B' : 'rgba(255,255,255,0.92)', borderRadius: 12, paddingVertical: 11, alignItems: 'center', borderWidth: 1, borderColor: active ? '#F59E0B' : 'rgba(255,255,255,0.6)' }}>
                <Text style={{ fontWeight: '900', fontSize: 8, textTransform: 'uppercase', letterSpacing: 0.5, color: active ? 'white' : '#6B7280' }}>{p.label}</Text>
              </TouchableOpacity>
            );
          })}
        </View>
          ...
          {(['all', 'income', 'expense'] as const).map(f => {
            ...
            return (
              <TouchableOpacity key={f} onPress={() => setTypeFilter(f)}
                style={{ flex: 1, backgroundColor: active ? '#F59E0B' : 'rgba(255,255,255,0.92)', borderRadius: 12, paddingVertical: 12, alignItems: 'center', borderWidth: 1, borderColor: active ? '#F59E0B' : 'rgba(255,255,255,0.6)' }}>
                <Text style={{ fontWeight: '900', fontSize: 10, textTransform: 'uppercase', letterSpacing: 1, color: active ? '#FFFFFF' : '#6B7280' }}>{f}</Text>
              </TouchableOpacity>
            );
          })}
      ...
      <TouchableOpacity
        ...
        style={{ borderRadius: BORDER_RADIUS, overflow: 'hidden', marginTop: 28 }}
      >
        <LinearGradient colors={EMERALD} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={{ padding: 16 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <View style={{ width: 40, height: 40, borderRadius: 14, backgroundColor: 'rgba(255,255,255,0.15)', alignItems: 'center', justifyContent: 'center', marginRight: 10 }}>
                <Image source={require('../../../assets/icons/wallet.png')} style={{ width: 30, height: 30 }} resizeMode="contain" />
              ...
              <View>
                ...
                <Text style={{ color: 'rgba(255,255,255,0.7)', fontSize: 8, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 1.5, marginTop: 2 }}>{selectedBranch?.name || 'Branch Account'}</Text>
              </View>
            </View>
          ...
          {pettyCashPending > 0 && (
            <Text style={{ color: '#FDE68A', fontSize: 9, fontWeight: '800', marginTop: 10, textTransform: 'uppercase', letterSpacing: 1 }}>
  ...
  const renderHistory = () => (
    <View style={{ marginTop: 28 }}>
      <View style={{ marginBottom: 14 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          <View>
            <Text style={{ fontWeight: '800', fontSize: 18, color: TEXT_PRIMARY }}>Transactions</Text>
            <Text style={{ fontWeight: '900', fontSize: 11, textTransform: 'uppercase', letterSpacing: 1, color: TEXT_MUTED, marginTop: 4 }}>{historyFiltered.length} entries</Text>
          </View>
          {isSelectMode ? (
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <TouchableOpacity onPress={exitSelectMode} style={{ paddingHorizontal: 14, paddingVertical: 9, borderRadius: 12, backgroundColor: 'rgba(255,255,255,0.92)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)' }}>
                <Text style={{ fontWeight: '900', fontSize: 10, textTransform: 'uppercase', letterSpacing: 1, color: '#6B7280' }}>Cancel</Text>
              </TouchableOpacity>
              {selectedIds.size > 0 && (
                <TouchableOpacity onPress={handleBatchDelete} style={{ paddingHorizontal: 14, paddingVertical: 9, borderRadius: 12, backgroundColor: '#EF4444' }}>
                  <Text style={{ fontWeight: '900', fontSize: 10, textTransform: 'uppercase', letterSpacing: 1, color: '#FFFFFF' }}>Delete {selectedIds.size}</Text>
                </TouchableOpacity>
              )}
            </View>
          ) : (
            <TouchableOpacity onPress={enterSelectMode} style={{ width: 40, height: 40, borderRadius: 12, backgroundColor: 'rgba(255,255,255,0.92)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {historyFiltered.length === 0 ? (
        <View style={{ alignItems: 'center', paddingVertical: 60, opacity: 0.4 }}>
          <Image source={require('../../../assets/icons/database.png')} style={{ width: 64, height: 64 }} resizeMode="contain" />
  ...
  const renderPending = () => (
    <View style={{ marginTop: 28 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <View>
          <Text style={{ fontWeight: '800', fontSize: 18, color: TEXT_PRIMARY }}>Pending Requests</Text>
          <Text style={{ fontWeight: '900', fontSize: 11, textTransform: 'uppercase', letterSpacing: 1, color: TEXT_MUTED, marginTop: 4 }}>
            {pendingCount} pending
          </Text>
        </View>
      </View>
      <View style={{ flexDirection: 'row', marginBottom: 16, gap: 8 }}>
        <View style={{ flex: 1, backgroundColor: '#D1FAE5', borderRadius: 14, padding: 12, alignItems: 'center' }}>
          <Text style={{ fontWeight: '900', fontSize: 14, color: '#065F46' }}>{rupee(pendingIncome)}</Text>
          <Text style={{ fontSize: 8, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 1, color: '#065F46', marginTop: 3 }}>Income</Text>
        </View>
        <View style={{ flex: 1, backgroundColor: '#FEE2E2', borderRadius: 14, padding: 12, alignItems: 'center' }}>
          <Text style={{ fontWeight: '900', fontSize: 14, color: '#991B1B' }}>{rupee(pendingExpense)}</Text>
          <Text style={{ fontSize: 8, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 1, color: '#991B1B', marginTop: 3 }}>Expense</Text>
        </View>
        <View style={{ flex: 1, backgroundColor: pendingNet >= 0 ? '#D1FAE5' : '#FEE2E2', borderRadius: 14, padding: 12, alignItems: 'center' }}>
          <Text style={{ fontWeight: '900', fontSize: 14, color: pendingNet >= 0 ? '#065F46' : '#991B1B' }}>{rupee(pendingNet)}</Text>
          <Text style={{ fontSize: 8, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 1, color: pendingNet >= 0 ? '#065F46' : '#991B1B', marginTop: 3 }}>Net</Text>
        </View>
      </View>
      {transactions.filter(t => t.status === 'pending').length === 0 ? (
        <View style={{ alignItems: 'center', paddingVertical: 60, opacity: 0.4 }}>
          <Image source={require('../../../assets/icons/exam-results.png')} style={{ width: 64, height: 64 }} resizeMode="contain" />
  ...
  const renderEntry = () => (
    <View style={{ marginTop: 28 }}>
      <Text style={{ fontWeight: '800', fontSize: 18, color: TEXT_PRIMARY, marginBottom: 16 }}>{editingItem ? (isAdmin ? 'Edit Request' : 'Edit Entry') : (isAdmin ? 'New Request' : 'New Entry')}</Text>
      <GlassCard style={{ padding: 20 }}>
        <View style={{ flexDirection: 'row', gap: 10, marginBottom: 20 }}>
          <TouchableOpacity onPress={() => setEntryType('income')}
            style={{ flex: 1, backgroundColor: entryType === 'income' ? '#10B9811F' : 'rgba(247,249,246,0.9)', borderRadius: 14, paddingVertical: 14, alignItems: 'center', borderWidth: 1, borderColor: entryType === 'income' ? '#10B981' : 'rgba(255,255,255,0.6)' }}>
            <Image source={require('../../../assets/icons/maths.png')} style={{ width: 30, height: 30 }} resizeMode="contain" />
            <Text style={{ fontWeight: '900', fontSize: 12, textTransform: 'uppercase', letterSpacing: 1, color: entryType === 'income' ? '#065F46' : '#6B7280', marginTop: 6 }}>Income</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => setEntryType('expense')}
            style={{ flex: 1, backgroundColor: entryType === 'expense' ? '#EF44441F' : 'rgba(247,249,246,0.9)', borderRadius: 14, paddingVertical: 14, alignItems: 'center', borderWidth: 1, borderColor: entryType === 'expense' ? '#EF4444' : 'rgba(255,255,255,0.6)' }}>
            <Image source={require('../../../assets/icons/wallet.png')} style={{ width: 30, height: 30 }} resizeMode="contain" />
            <Text style={{ fontWeight: '900', fontSize: 12, textTransform: 'uppercase', letterSpacing: 1, color: entryType === 'expense' ? '#991B1B' : '#6B7280', marginTop: 6 }}>Expense</Text>
          </TouchableOpacity>
        </View>

        <View style={{ marginBottom: 16 }}>
          <Text style={{ fontSize: 10, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 2, color: TEXT_MUTED, marginBottom: 8 }}>Description</Text>
          <View style={{ backgroundColor: 'rgba(247,249,246,0.9)', borderRadius: 16, padding: 16, borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', flexDirection: 'row', alignItems: 'center' }}>
            <MaterialCommunityIcons name="text-box-outline" size={20} color="#F59E0B" style={{ marginRight: 12 }} />
            <TextInput value={name} onChangeText={setName} placeholder="e.g. Term Fees, Salary..." placeholderTextColor="#9CA3AF"
              style={{ flex: 1, fontWeight: '900', fontSize: 15, color: TEXT_PRIMARY }} />
          </View>
        </View>

        <View style={{ flexDirection: 'row', gap: 10, marginBottom: 16 }}>
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 10, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 2, color: TEXT_MUTED, marginBottom: 8 }}>Amount</Text>
            <View style={{ backgroundColor: 'rgba(247,249,246,0.9)', borderRadius: 16, padding: 16, borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)' }}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 10, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 2, color: TEXT_MUTED, marginBottom: 8 }}>Category</Text>
            <TouchableOpacity onPress={() => setShowCategoryModal(true)}
              style={{ backgroundColor: 'rgba(247,249,246,0.9)', borderRadius: 16, padding: 16, borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
              <Text style={{ fontWeight: '900', fontSize: 11, textTransform: 'uppercase', letterSpacing: 1, color: category ? TEXT_PRIMARY : '#9CA3AF' }}>{category || 'SELECT'}</Text>
        ...
        <View style={{ marginBottom: 16 }}>
          <Text style={{ fontSize: 10, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 2, color: TEXT_MUTED, marginBottom: 8 }}>Payment Method</Text>
          <View style={{ flexDirection: 'row', gap: 6 }}>
            {paymentMethods.map(m => {
              const active = paymentMethod === m;
              return (
                <TouchableOpacity key={m} onPress={() => { setPaymentMethod(m); if (m === 'Petty Cash') showPettyCashInfo(); }}
                  style={{ flex: 1, backgroundColor: active ? '#10B981' : 'rgba(247,249,246,0.9)', borderRadius: 12, paddingVertical: 10, alignItems: 'center', borderWidth: 1, borderColor: active ? '#10B981' : 'rgba(255,255,255,0.6)' }}>
                  <MaterialCommunityIcons name={m === 'Cash' ? 'cash' : m === 'Bank' ? 'bank' : m === 'UPI' ? 'qrcode' : 'wallet-outline'} size={16} color={active ? '#FFFFFF' : '#6B7280'} />
                  <Text style={{ fontWeight: '800', fontSize: 8, textTransform: 'uppercase', letterSpacing: 0.5, color: active ? '#FFFFFF' : '#6B7280', marginTop: 4 }}>{m}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
          {isPettyCash && (
            <View style={{ marginTop: 10, backgroundColor: pettyCashBalance !== null ? (balanceOk ? '#FEE2E2' : '#DCFCE7') : 'rgba(247,249,246,0.9)', borderRadius: 14, padding: 14 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                <Text style={{ fontWeight: '700', fontSize: 12, color: TEXT_PRIMARY }}>Petty Cash Available</Text>
              ...
              {pettyCashPending > 0 && (
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 6 }}>
                  <Text style={{ fontWeight: '700', fontSize: 12, color: '#B45309' }}>Pending approval</Text>
                  <Text style={{ fontWeight: '900', fontSize: 14, color: '#F59E0B' }}>-₹{pettyCashPending.toLocaleString('en-IN')}</Text>
                </View>
              )}
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 6 }}>
                <Text style={{ fontWeight: '700', fontSize: 12, color: TEXT_PRIMARY }}>Balance after this</Text>
              ...
              {balanceOk && (
                <View style={{ backgroundColor: '#FEE2E2', borderRadius: 8, padding: 8, marginTop: 10 }}>
                  <Text style={{ color: '#991B1B', fontWeight: '800', fontSize: 10, textAlign: 'center' }}>Insufficient Petty Cash Balance</Text>
                </View>
              )}
              {isAdmin && (
                <Text style={{ color: '#9CA3AF', fontWeight: '700', fontSize: 9, marginTop: 8, textTransform: 'uppercase', letterSpacing: 0.5 }}>
        ...
        <View style={{ marginBottom: 20 }}>
          <DatePicker label="Date" value={entryDate} onChange={setEntryDate} />
        </View>

        <TouchableOpacity onPress={handleSubmit} disabled={isSubmitting} activeOpacity={0.9} style={{ borderRadius: 18, overflow: 'hidden' }}>
          <LinearGradient colors={AMBER} style={{ paddingVertical: 16, alignItems: 'center', justifyContent: 'center', flexDirection: 'row' }}>
      ...
      <ChoiceModal visible={showCategoryModal} onClose={() => setShowCategoryModal(false)} title="Category" message="" iconName="layers-triple" accentColor="#F59E0B" options={[
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F7F9F6' }}>
      ...
      <ScrollView
        ...
        contentContainerStyle={{ paddingTop: Math.max(insets.top, 56), paddingBottom: 120 }}
      >
        <View style={{ paddingHorizontal: 20 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            <View style={{ flex: 1, marginLeft: 14 }}>
              ...
              <Text style={{ fontSize: 22, fontWeight: '700', color: TEXT_PRIMARY, marginTop: 2, letterSpacing: -0.5 }}>Finance</Text>
            </View>
            <View style={{ width: 54, height: 54, borderRadius: 18, backgroundColor: 'rgba(255,255,255,0.92)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.6)', alignItems: 'center', justifyContent: 'center' }}>
              <Image source={require('../../../assets/icons/wallet.png')} style={{ width: 44, height: 44 }} resizeMode="contain" />
            </View>
          </View>

          <View style={{ marginTop: 16, alignSelf: 'flex-start', backgroundColor: 'rgba(245,158,11,0.14)', paddingHorizontal: 14, paddingVertical: 6, borderRadius: 100, borderWidth: 1, borderColor: 'rgba(245,158,11,0.2)', flexDirection: 'row', alignItems: 'center' }}>
            <Image source={require('../../../assets/icons/exam-results.png')} style={{ width: 18, height: 18 }} resizeMode="contain" />
            <Text style={{ color: '#D97706', fontSize: 10, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 2, marginLeft: 8 }}>Income & Expense</Text>
          </View>
          ...
          <View style={{ flexDirection: 'row', gap: 8, marginTop: SECTION_GAP }}>
            {([
              { key: 'history' as const, label: 'Ledger' },
              { key: 'pending' as const, label: `Pending${pendingCount > 0 ? ` (${pendingCount})` : ''}` },
              { key: 'entry' as const, label: isAdmin ? 'Request' : 'Entry' },
            ]).map(tab => {
              ...
              return (
                <TouchableOpacity
                  ...
                  style={{
                    flex: 1,
                    backgroundColor: active ? '#F59E0B' : 'rgba(255,255,255,0.92)',
                    borderRadius: 14,
                    paddingVertical: 12,
                    alignItems: 'center',
                    borderWidth: 1,
                    borderColor: active ? '#F59E0B' : 'rgba(255,255,255,0.6)',
                  }}
                >
                  <Text style={{ fontWeight: '900', fontSize: 10, textTransform: 'uppercase', letterSpacing: 1, color: active ? 'white' : '#6B7280' }}>
                    {tab.label}
                  </Text>
                </TouchableOpacity>
              );
